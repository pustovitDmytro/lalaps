...
import originalV1report from './npm-reports/original.v1.json';
import originalV2report from './npm-reports/original.v2.json';
...
test('parse auditReportVersion 2', async function () {
    assert.deepEqual(
        ...
        {
            'advisories' : [
                { 'id': `npm_${1_006_254}`, 'title': 'Regular Expression Denial of Service in csv-parse', 'url': 'https://github.com/advisories/GHSA-582f-p4pg-xc74', 'severity': 'high', 'range': '<4.4.6', 'vulnerableLibrary': 'csv-parse', 'fix': '5.0.3', 'rootLibraries': [ { 'name': 'csv-parse', 'range': '<4.4.6', 'fix': '5.0.3' } ] },
...
test('parse auditReportVersion 1', async function () {
    assert.deepEqual(
        NPM.parseReport(originalV1report),
        {
            'advisories' : [
...
test('parse original auditReportVersion 2', async function () {
    assert.deepEqual(
        NPM.parseReport(originalV2report),
        {
            'advisories' : [
                { 'id': 'npm_1004946', 'title': ' Inefficient Regular Expression Complexity in chalk/ansi-regex', 'url': 'https://github.com/advisories/GHSA-93q8-gq69-wqmw', 'severity': 'moderate', 'range': '>2.1.1 <5.0.1', 'vulnerableLibrary': 'ansi-regex', 'rootLibraries': [ ], fix: true },
                // SHOULD BE 'rootLibraries': [ { 'name': 'semantic-release' } ]

                { 'id': 'npm_1006724', 'title': 'json-schema is vulnerable to Prototype Pollution', 'url': 'https://github.com/advisories/GHSA-896r-f27r-55mw', 'severity': 'moderate', 'range': '<0.4.0', 'vulnerableLibrary': 'json-schema', 'fix': null, 'rootLibraries': [ { 'name': 'coveralls', 'range': '>=2.11.5', 'fix': '2.11.4' }, { 'name': 'semantic-release-heroku', 'range': '*', 'fix': null } ] },

                { 'id': 'npm_1004869', 'title': 'Prototype Pollution in node-jsonpointer', 'url': 'https://github.com/advisories/GHSA-282f-qqgm-c34q', 'severity': 'moderate', 'range': '<5.0.0', 'vulnerableLibrary': 'jsonpointer', 'fix': '0.13.0', 'rootLibraries': [ { 'name': 'danger', 'range': '>=0.14.0', 'fix': '0.13.0' } ] }
            ],
            'meta' : {
                'vulnerabilities' : { 'total': 22, 'categories': { 'info': 0, 'low': 0, 'moderate': 22, 'high': 0, 'critical': 0 } },
                'dependencies'    : { 'total': 1588, 'categories': { 'prod': 219, 'dev': 1370, 'optional': 6, 'peer': 0, 'peerOptional': 0 } }
            }
        }
    );
});
